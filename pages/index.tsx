...
const Form = ({ name, instance, onNameChange, onInstanceChange }: any) => (
  <form className='flex flex-col gap-4'>
    ...
    <Label className='dark:text-gray-700'>
      <span>Instance</span>
      <Input className="mt-1 dark:bg-white dark:text-gray-800 dark:border-gray-400" onChange={(e) => onInstanceChange(e.target.value)} value={instance} crossOrigin={undefined} />
    </Label>
  </form>
)
...
const Home: NextPage = () => {
  ...
  const [customer, setCustomer] = useState<CustomerAbstract>({ name: "", instance: "" })
  ...
  const checkNextPrevious = () => {
    if (indexQuestion == -1) {
      if (!customer.name || !customer.instance) {
        setIsNext(false);
      } else {
        setIsNext(true);
      }
        ...
        <Card className="mb-8 shadow-md dark:bg-white ring-gray-300 p-0  md:w-2/5 w-full  shadow-xl">
          <CardBody className='bg-white p-10 md:px-10'>
            ...
            {!isSubmited ? (
              <div className="mt-8">
                {indexQuestion == -1 ? (
                  < Form name={customer.name} instance={customer.instance} onNameChange={(name: string) => setCustomer({ ...customer, name })} onInstanceChange={(instance: string) => setCustomer({ ...customer, instance })} />
                ) : (
                  <Question />
                )}
                <div className="flex">
